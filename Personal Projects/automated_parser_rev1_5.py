...
def csvWrite_BIN(namepart, file_count, pdir, gyro_x, gyro_y, gyro_z, acc_x, acc_y, acc_z):
    ...
    print("Writing" + str(pdir) + "/sdd_bin_" + str(time_fmt) + "_" + str(namepart) + ".csv\n")
    with open(str(pdir) + "/sdd_bin_" + str(time_fmt) + "_" + str(namepart) + ".csv", 'w+', newline="") as csvfile:
        fieldnames = ["gyro_x", "gyro_y", "gyro_z", "accel_x", "accel_y", "accel_z"]
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        
        for i in range(gyro_x):
            try:
...
def csvWrite_Smoke(timestamps, pdir, mc1p0, mc2p5, mc4p0, mc10p0, nc0p5, nc1p0, nc2p5, nc4p0, nc10p0):

    print("Writing " + str(pdir) + "/smoke_data_" + str(time_fmt) + ".csv\n")
    
    with open(str(pdir) + "/smoke_data_" + str(time_fmt) + ".csv", 'w+', newline="") as csvfile:
...
def parseFiles(parseRawFlag):
    ...
    path = ["0_H", "0_V", "90_H", "90_V", "180_H", "180_V", "270_H", "270_V"] 
    loop_count = 0
    count = 0
    file_count = 0
    while loop_count < len(path):
        try:
            for pdir in path:
                pdir = str(pdir)
                times = parseLog(str(pdir),str(":SDD"))
                
                for filename in os.listdir(str(pdir)):
                    
                    if filename.endswith(".bin"):
                        print("Extracting from " + pdir + "/" + filename)
                        data = np.fromfile(os.path.join(pdir, filename), dtype=fmt)
                        ...
                        csvWrite_BIN(namepart, file_count, pdir, gyro_x, gyro_y, gyro_z, acc_x, acc_y, acc_z)
                        ...
                        with open(str(pdir) + "/halt.log", "r+") as log:
                            ...
                            csvWrite_Smoke(times, pdir, mc10p0, mc2p5, mc4p0, mc10p0, nc0p5, nc1p0, nc2p5, nc4p0, nc10p0)              
                    
                    elif filename.endswith(".raw") and parseRawFlag:

                        try: 
                            data, samplerate = sf.read(str(pdir) + "/" + filename, channels=2, samplerate=44100, subtype="FLOAT")
                            namepart = filename[:7]

                            with open(str(pdir) + "/sdd_raw_" + str(time_fmt) + "_" + namepart + ".csv", 'w+', newline="") as csvfile:
                                fieldnames = ["left_channel", "right_channel"]
                                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                                writer.writeheader()

                                print("Get comfy, because this is going to be a while...")
                                print("Writing " + str(pdir) + "sdd_raw_" + str(time_fmt) + "_" + namepart + ".csv to file.")
            ...
            print("You should have a folder each with the following names: 0_H, 0_V, 90_H, 90_V, 180_H, 180_V, 270_H, 270_H\n")
            break
    
    if count > len(path):
        print("\nSuccess! All files have been extracted, parsed, and converted to CSV. You may now close this script/terminal.\n")

def parseLog(pdir, patt):
